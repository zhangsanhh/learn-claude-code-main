...
class WorktreeManager:
    ...
    def create(
        ...
        try:
            self._run_git(["worktree", "add", "-b",
                          branch, str(path), base_ref])
    ...
    def remove(self, name: str, force: bool = False,
        ...
        try:
            ...
            self.events.emit(
                "worktree.remove.failed", task={
                    "id": wt.get("task_id")} if wt.get("task_id") is not None else {}, worktree={
                    "name": name, "path": wt.get("path")}, error=str(e), )
            raise
    ...
    def keep(self, name: str) -> str:
        ...
        return json.dumps(
            kept, indent=2) if kept else f"Error: Unknown worktree '{name}'"
