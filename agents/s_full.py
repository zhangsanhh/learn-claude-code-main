    ...
    def update(self, items: list) -> str:
        ...
        for i, item in enumerate(items):
            ...
            if status == "in_progress":
                ip += 1
            validated.append(
                {"content": content, "status": status, "activeForm": af})
        if len(validated) > 20:
...
def run_subagent(prompt: str, agent_type: str = "Explore") -> str:
    ...
    if resp:
        return "".join(
            b.text for b in resp.content if hasattr(
                b, "text")) or "(no summary)"
...
class TaskManager:
    ...
    def _save(self, task: dict):
        (TASKS_DIR /
         f"task_{task['id']}.json").write_text(json.dumps(task, indent=2))
    ...
    def list_all(self) -> str:
        ...
        for t in tasks:
            ...
            blocked = f" (blocked by: {t['blockedBy']})" if t.get(
                "blockedBy") else ""
...
class BackgroundManager:
    ...
    def check(self, tid: str = None) -> str:
        ...
        return "\n".join(f"{k}: [{v['status']}] {v['command'][:60]}" for k,
                         v in self.tasks.items()) or "No bg tasks."
...
class MessageBus:
    ...
    def read_inbox(self, name: str) -> list:
        ...
        msgs = [json.loads(line)
                for line in path.read_text().strip().splitlines() if line]
        path.write_text("")
        return msgs
    ...
    def _loop(self, name: str, role: str, prompt: str):
                                                                                                ...
                                                                                                "type": "object", "properties": {
                                                                                                    "to": {
                                                                                                        "type": "string"}, "content": {
                                                                                                            "type": "string"}}, "required": [
                                                                                                                "to", "content"]}}, {
                                                                                                                    "name": "idle", "description": "Signal no more work.", "input_schema": {
                                                                                                                        "type": "object", "properties": {}}}, {
                                                                                                                            "name": "claim_task", "description": "Claim task by ID.", "input_schema": {
                                                                                                                                "type": "object", "properties": {
                                                                                                                                    "task_id": {
        ...
        while True:
                        ...
                        if block.name == "idle":
                            ...
                            dispatch = {"bash": lambda **kw: run_bash(kw["command"]),
                                        ...
                                        "edit_file": lambda **kw: run_edit(kw["path"],
                                                                           ...
                                                                           kw["new_text"])}
                            output = dispatch.get(
                                block.name, lambda **kw: "Unknown")(**block.input)
            ...
            for _ in range(IDLE_TIMEOUT // max(POLL_INTERVAL, 1)):
                ...
                for f in sorted(TASKS_DIR.glob("task_*.json")):
                    t = json.loads(f.read_text())
                    if t.get("status") == "pending" and not t.get(
                            "owner") and not t.get("blockedBy"):
                        unclaimed.append(t)
                if unclaimed:
                    task = unclaimed[0]
